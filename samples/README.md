...
| `ProTranslate.Uno.Sample` | Non-Windows project stub only | Windows sample job | Validates Windows-hosted Uno `x:Bind` and attached `Translation.Key` paths with the modern `win-x64` Windows App SDK RID. The adapter emits a shared URI mapping, while the sample keeps WinUI-compatible `using:` syntax. Skia/WebAssembly runtime validation is planned. |
